...
require_relative 'revision_info'
...
module Release
  class CLI < Thor
    ...
    desc "increment <MODULE_NAME>", "Increment minor revision index"
    def increment(module_name = nil)
      _, revision_info = load(module_name)
      puts "Incrementing #{revision_info} to #{revision_info.minor_increment!}"
      options[:dryrun] ? revision_info.write_to_file : revision_info.write!
    end

    desc "release <MODULE_NAME>", "Increment major revision index"
    def release(module_name = nil)
      _, revision_info = load(module_name)
    ...
    def archive(module_name = nil)
      ...
      Zip::File.open(zipfile_name, Zip::File::CREATE) do |zipfile|
        ...
        if module_info.is_bootloadable
          ...
          # bootloader_revision_info = RevisionInfo.new(module_info.get_bootloader_info)
    ...
    def load(module_name)
      ...
      begin
        revision_info = RevisionInfo.new(module_info);
